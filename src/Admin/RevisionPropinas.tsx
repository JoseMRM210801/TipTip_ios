...
export const RevisionPropinas = () => {
    ...
    const idiomaSpanol = {
        ...
        pregunta: 'Propinas por cliente',
        propuesta: 'Total propinas: '

    }
    const idiomaIngles = {
        salir: 'Exit',
        saludo: "Hello admistrator",
        pregunta: 'Tips per client',
        propuesta: 'Total tips: '
    };
            ...
            .then(datos => {
                if (datos) {
                    ...
                    datos.forEach((element: any) => {
                        totalAuxiliar += parseFloat(element[2]);
                    });
                        ...
                        <View>
                            {
                                data?.map((row, index) => (
                                    <TouchableOpacity
                                        onPress={() => {
                                            if (index > 0) {
                                                navigation.navigate('DeatllesPropinaCliente', { id: row[0].toString() })
                                            }
                                        }}
