...
export const DetallesPropinasRepartidor = () => {
  ...
  const idiomaSpanol = {
    ...
    revisionpropina:'Revision de propinas',
    pagar:'Pagar'
  }
  const idiomaIngles = {
    salir: 'Exit',
    ...
    revisionpropina:'Tip Review',
    pagar:'Pay'
  };
  useEffect(() => {
    const id = route.params?.id;
    // fetch(`https://bett-production.up.railway.app/api/tip/delivery/${id}`, {
      fetch(`${Ruta}/tip/delivery/${id}`, {
      ...
      .then(datos => {
        if (datos) {
          ...
          datos.unshift(['#', 'Cliente', 'Propinas Totales','Propinas despues de paypal','paypal','Fecha']);
